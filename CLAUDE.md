...
- **seal**(`packages/seal`,自研):PDF 电子盖章——`seal_stamp`(合同章:指定页面、锚点/毫米坐标、按图片比例保持圆形不变形)与 `seal_straddle`(骑缝章:一枚章按组内页数等分,每页边缘一条)。分层:`lib/geometry.js`(纯算术:毫米↔点、锚点、越界判定、页码选择器、分组、切片窗口)、`lib/pdf.js`(pdf-lib 操作)、`lib/index.js`(工具注册)。要点:
  ...
  - **抠背景与自动定位都集成在盖章工具内部**(用户要求「能力要集成到插件里面」,不是让人先手动处理):
    - **背景色是检测出来的,不能假设是白的**(踩过):第一版按「各通道都亮 = 背景」写,遇到用户真实的**绿底紫章**直接失效(纯绿 min(R,G,B)≈0,会被整块保留)。现在采样图片边缘、取众数当背景色,再按颜色距离做带渐变的抠除——白纸底/绿幕底/灰底都成立,墨是什么颜色都行。`inkOnly`(只留红)默认关闭且必须保持关闭,那枚真实印章是紫色的。抠完报出底色与保留比例,几乎全留/几乎全抠都明确警告(印出来之前看不见)。
    - **位置靠读文档自己的文字定**(`anchor: 'auto'` 为默认):`pdftotext -bbox`(poppler,可选)拿到每个词的坐标,按「盖章/签章 > signature > 签字 > 甲方」加权,并偏向靠后的页与靠下的位置。实测用户那份 5 页协议,签署块在**第 4 页**——固定「最后一页右下角」会盖到空白页上。`party` 限定时**绝不落到对方那一栏**。poppler 缺席或找不到关键词时退回锚点并在结果里说明,而不是假装找过。
    - **坐标系必须换算**:pdftotext 是左上原点,PDF 绘制是左下原点;弄反了会盖到「正好对称的错误位置」,看起来还挺合理,所以有专门的断言。
  - **合同多半是 .docx,所以有 `seal_to_pdf`(LibreOffice)**:转换是**明示的一步**,不由盖章工具顺手做——它会替换字体、改变分页,而**转换之后那份 PDF 才是"文件"**(被盖章、被签名、争议时被拿出来的那份)。盖章/签名工具遇到非 PDF 会拒绝并**指向这个工具**(单说"不是 PDF"正确但没用)。LibreOffice 不随包分发,找不到时明确报错并给出安装/配置办法。
  - **两个启动崩溃(都已修,都加了守卫)**:①`ctx.systemPrompt.section()` 的 **`order` 必填且必须是有限数**,漏了会 `TypeError` → apply 抛错 → **整棵插件树加载失败、服务退出 code=1**,桌面壳只会说「最后装的那个插件嫌疑最大」。vision 当时有同样的洞。合规检查现在照抄真实服务那一条 `Number.isFinite` 断言,假 ctx 不再什么都收。②**只被某个可选功能用到的服务不要写进顶层 `inject`**:`webServer` 只有客户端设置路由要用,写进顶层会让没有 Web 服务的 profile(headless/CLI)整个插件**永不激活**,启动报 `pending (waiting for service: webServer)` 而失败——盖章签名一起没了。改成嵌套 `ctx.inject(['webServer'], …)`。和 Tushare 那条是同一个教训。
  - **密钥由客户端配置**(用户要求):Web 客户端 设置 → 签章证书 一页表单,经宿主路由 `/seal/credential`(GET/POST/DELETE)把凭证存进 **storage 域**,而不是 profile 的 `cordis.patch.yml`——那个文件会被备份/同步/误提交/贴进 issue。**GET 永不返回口令**,只返回 `hasPassphrase`(返回口令等于塞进每个浏览器缓存与 devtools 日志)。提交时拒绝 `.cer`(公钥半边不能签名,否则报错会是 ASN.1 那种驴唇不对马嘴的)。这是本仓库第一个 `tools/` 分类却带客户端半边的包(`dsh.client` + 手写 factory bundle,loader id 必须等于包名)。诚实边界:**存储域不是密钥串**,本机未加密、同用户进程可读;它买到的是「口令不再跟着配置文件到处走」。
  - **签名凭证也可从别处来,但口令的去处要说清**(用户要求):`p12Path` / `passphraseEnv` / `passphrase` 三选一,取用顺序是**调用参数 → 环境变量 → 配置明文**。`passphraseEnv` 指定了但变量没设**直接报错**,不退回明文那个——否则你以为在用 A 口令、实际用的是 B。**客户端表单不认 `role: 'secret'`**(查证过 `dsh-client-schema-form`),所以明文口令在界面上也不打码,配了明文口令启动时会打一条日志提醒;结果里只写口令**来源**不写值。
